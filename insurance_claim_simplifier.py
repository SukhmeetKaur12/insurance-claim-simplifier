# -*- coding: utf-8 -*-
"""insurance claim simplifier.ipynb
...
pdf_folder = "/content/insurance data"
...
query = "Is ambulance covered under my policy?"
answer = generate_answer(query)
print("Answer:", answer)
...
"""Factors to Consider for Approval Logic:
Feature	Real-World Relevance	Suggested Condition
claim	High claims may be denied or require more scrutiny	Approve if claim < 15000
smoker	Smokers are higher risk	Deny if smoker == 'Yes' and claim > 10000
bmi	Very high or low BMI may indicate health risks	Deny if bmi > 40 or bmi < 16
age	Older people may be scrutinized more	Deny if age > 60 and claim > 12000
diabetic	Diabetics may have more complications	Deny if diabetic == 'Yes' and claim > 13000
bloodpressure	Very high BP is a risk	Deny if bloodpressure > 140
children	May increase coverage	Approve if children >= 2 and claim < 15000
...
# Show the total NaN values per column
print("NaN values per column:")
print(df.isna().sum())

""". Fill NaNs with default values
Age can be filled with mean or median
Region (categorical) can be filled with mode (most frequent value)
...
print("approved" in X.columns)  # Should return False

# remove if not already done

print(y.value_counts(normalize=True))# here we are cheacking the class balance

import seaborn as sns
import matplotlib.pyplot as plt

corr_matrix = df[['claim', 'approved']].corr()
sns.heatmap(corr_matrix, annot=True, cmap='coolwarm')
plt.title("Correlation Matrix between 'claim' and 'approved'")
plt.show()

import seaborn as sns
import matplotlib.pyplot as plt

# Make sure 'approved' is in the DataFrame
corr_matrix = df.corr(numeric_only=True)

plt.figure(figsize=(10, 6))
sns.heatmap(corr_matrix[['approved']].sort_values(by='approved', ascending=False), annot=True, cmap='coolwarm')
plt.title("Correlation of Features with 'approved'")
plt.show()

print(df[['claim', 'approved']].corr())
...
df_clean = df.drop(['index', 'PatientID'], axis=1)

# Separate target variable
y = df_clean['approved']
X = df_clean.drop('approved', axis=1)

# One-hot encode categorical variables (optional: drop_first=True)
X = pd.get_dummies(X, drop_first=True)

# Train-test split AFTER all cleaning
from sklearn.model_selection import train_test_split
...
folder_path = "/content/insurance data"
documents = []
...
"""FROM HERE WE WILL INTEGRATE THE FLAN T5 MODEL RAG AND THE PREDICTION MODEL AND ALSO BUILD OUR UI
